...
namespace FF4FalconDive.Inventory
{
	public static class Messages
	{
		...
		public static void updateMessages(string directory, string seed, string flags, string checksum, bool sirensAllowed, int nothings, int[] party, TextBox[,] heroNames, Random r1)
		{
			List<Message> msgStrings;
			string keyNothing = (nothings / 2) + " Nothing" + (nothings >= 4 || nothings <= 1 ? "s" : "");
			string itemNothing = nothings <= 1 ? "99 Nothings" : (int)Math.Ceiling((double)nothings * 3 / 4) + " Nothings";
			...
			foreach (string language in languages)
			{
				...
				using (StreamReader reader = new StreamReader(Updater.MemoriaToMagiciteFile(directory, "Message\\story_mes_" + language + ".txt")))
				{
					...
					msgStrings.Add(new Message { id = "FE_NOTHING_G1", msgString = $"You now have found {keyNothing}!  Check out the Nothing Vending Machine in the Agart Laboratory (the telescope) to receive a possible key item!" });
					msgStrings.Add(new Message { id = "FE_NOTHING_G2", msgString = $"You now have found {itemNothing}!  Check out the Nothing Vending Machine in the Agart Laboratory (the telescope) for a special reward!" });
					...
					msgStrings.Add(new Message { id = "FE_NM_L1_GOOD", msgString = $"This is the Nothing Vending Machine.  You have received at least {keyNothing}!  Dispensing prize..." });
					msgStrings.Add(new Message { id = "FE_NM_L2_GOOD", msgString = $"This is the Nothing Vending Machine.  You have received at least {itemNothing}!  Dispensing your special prize..." });
					msgStrings.Add(new Message { id = "FE_NM_L1_BAD", msgString = $"This is the Nothing Vending Machine.  You need at least {keyNothing} to receive a prize." });
					msgStrings.Add(new Message { id = "FE_NM_L2_BAD", msgString = $"This is the Nothing Vending Machine.  You need at least {itemNothing} to receive a special prize." });
